<template>
  <div>
    ...
    <el-form ref="searchform" inline size="small" :model="searchMap">
      <!-- <el-form-item label="ip白名单编号">
        <el-input v-model="searchMap.ipwhitelistid" prop="ipwhitelistid" clearable placeholder="ip白名单编号" /></el-form-item> -->
...
<script>
import ipportwhitelistApi from '@/api/ipportwhitelist'
  ...
  methods: {
    ...
    getPortList(query) {
      if (query !== '' && query) {
        ...
        setTimeout(() => {
          this.searchLoading = false
          ipportwhitelistApi.search(1, 10, { 'port': query }).then(response => {
    ...
    handleDeleteAll() {
      if (this.multipleSelection && this.multipleSelection.length) {
        this.$confirm('此操作将永久删除已选记录, 是否继续?', '警告', {
          ...
          ipportwhitelistApi.deleteAllByIds(ids).then(response => {
            this.$message({
    ...
    fetchData() {
      ...
      ipportwhitelistApi.search(this.currentPage, this.pageSize, this.searchMap).then(response => {
    ...
    handleSave() {
      ipportwhitelistApi.update(this.id, this.pojo).then(response => {
        this.$message({
          message: response.message,
          type: (response.flag ? 'success' : 'error')
        })
    ...
    handleDelete(id) {
      this.$confirm('此操作将永久删除已选记录, 是否继续?', '警告', {
        ...
        ipportwhitelistApi.deleteById(id).then(response => {
          this.$message({
            message: response.message,
            type: (response.flag ? 'success' : 'error')
          })
