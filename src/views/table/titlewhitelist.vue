  ...
  <div>
    ...
    <el-form ref="searchform" inline size="small" :model="searchMap">
      ...
      <el-form-item prop="title" label="标题">
        <el-select v-model="searchMap.title" allow-create default-first-option filterable remote clearable placeholder="请输入关键词搜索并手动选择" :remote-method="getTitleList" :loading="searchLoading">
      ...
      <el-table-column sortable prop="title" label="标题">
        <template slot="header">
          ...
          <el-tooltip placement="top">
            <div slot="content">如果web页面的title在标题白名单中<br>则检测结果中的web信息不记录该页面的body内容</div>
    ...
    <el-dialog title="编辑" :visible.sync="dialogFormVisible" width="50%" center :before-close="cleanCache">
      <el-form label-width="110px">
        ...
        <!-- <el-form-item label="标题"><el-input v-model="pojo.title" style="width:400px;" /></el-form-item> -->

        <el-form-item prop="title" label="标题">
          <el-select v-model="pojo.title" allow-create default-first-option filterable remote clearable placeholder="请输入关键词搜索并手动选择" :remote-method="getTitleList" :loading="searchLoading">
            <el-option v-for="item in titleList" :key="item.id" :label="item.title" :value="item.title" />
          </el-select>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    getTitleList(query) {
      if (query !== '' && query) {
        ...
        setTimeout(() => {
          ...
          titlewhitelistApi.search(1, 10, { 'title': query }).then(response => {
            this.titleList = response.data.rows.filter(item => {
    ...
    handleDeleteAll() {
      if (this.multipleSelection && this.multipleSelection.length) {
        this.$confirm('此操作将永久删除已选记录, 是否继续?', '警告', {
          ...
          titlewhitelistApi.deleteAllByIds(ids).then(response => {
            this.$message({
    ...
    fetchData() {
      ...
      titlewhitelistApi.search(this.currentPage, this.pageSize, this.searchMap).then(response => {
    ...
    handleEdit(id) {
      ...
      if (id !== '') { // 修改
        titlewhitelistApi.findById(id).then(response => {
          if (response.flag) {
    ...
    handleDelete(id) {
      this.$confirm('此操作将永久删除已选记录, 是否继续?', '警告', {
        ...
        titlewhitelistApi.deleteById(id).then(response => {
          this.$message({
            message: response.message,
            type: (response.flag ? 'success' : 'error')
          })
