  ...
  <div style="padding:5px;">
    ...
    <el-form ref="searchform" inline size="small" :model="searchMap">
      <el-form-item label="项目名称">
        <el-input v-model="searchMap.name" prop="name" clearable placeholder="项目名称" /></el-form-item>
      <el-form-item label="项目描述">
        <el-input v-model="searchMap.description" prop="description" clearable placeholder="项目描述" /></el-form-item>
    ...
    >
      ...
      <!-- <el-table-column sortable prop="id" label="项目编号" /> -->
      <el-table-column sortable prop="name" label="项目名称" />
      <el-table-column sortable prop="description" label="项目描述" />
...
export default {
  ...
  methods: {
    ...
    handleDeleteAll() {
      if (this.multipleSelection && this.multipleSelection.length) {
        this.$confirm('此操作将永久删除已选记录, 是否继续?', '警告', {
          ...
          projectApi.deleteAllByIds(ids).then(response => {
            this.$message({
    ...
    fetchData() {
      ...
      projectApi.search(this.currentPage, this.pageSize, this.searchMap).then(response => {
    ...
    handleSave() {
      projectApi.update(this.id, this.pojo).then(response => {
        this.$message({
          message: response.message,
          type: (response.flag ? 'success' : 'error')
        })
    ...
    handleEdit(id) {
      ...
      if (id !== '') { // 修改
        projectApi.findById(id).then(response => {
          if (response.flag) {
    ...
    handleDelete(id) {
      this.$confirm('此操作将永久删除已选记录, 是否继续?', '警告', {
        ...
        projectApi.deleteById(id).then(response => {
          this.$message({
            message: response.message,
            type: (response.flag ? 'success' : 'error')
          })
