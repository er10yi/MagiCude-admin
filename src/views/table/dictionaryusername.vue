  ...
  <div style="padding:5px;">
    ...
    <el-form ref="searchform" inline size="small" :model="searchMap">
      ...
      <el-form-item prop="username" label="用户名">
        <el-select v-model="searchMap.username" style="width:300px;" filterable remote allow-create default-first-option clearable placeholder="请输入关键词" :remote-method="getUsernameList" :loading="searchLoading">
    ...
    <el-dialog title="编辑" :visible.sync="dialogFormVisible" width="50%" center :before-close="cleanCache">
      <el-form label-width="100px">
        ...
        <el-form-item prop="username" label="用户名">
          <el-select v-model="pojo.username" style="width:300px;" filterable remote allow-create default-first-option clearable placeholder="请输入关键词" :remote-method="getUsernameList" :loading="searchLoading">
            <el-option v-for="item in usernameList" :key="item.id" :label="item.username" :value="item.username" /></el-select>
        </el-form-item>
...
<script>
import dictionaryusernameApi from '@/api/dictionaryusername'
...
export default {
  ...
  methods: {
    getUsernameList(query) {
      if (query !== '' && query) {
        ...
        setTimeout(() => {
          ...
          dictionaryusernameApi.search(1, 10, { 'username': query }).then(response => {
    ...
    handleDeleteAll() {
      if (this.multipleSelection && this.multipleSelection.length) {
        this.$confirm('此操作将永久删除已选记录, 是否继续?', '警告', {
          ...
          dictionaryusernameApi.deleteAllByIds(ids).then(response => {
            this.$message({
    ...
    fetchData() {
      ...
      dictionaryusernameApi.search(this.currentPage, this.pageSize, this.searchMap).then(response => {
    ...
    handleSave() {
      dictionaryusernameApi.update(this.id, this.pojo).then(response => {
        this.$message({
          message: response.message,
          type: (response.flag ? 'success' : 'error')
        })
    ...
    handleEdit(id) {
      ...
      if (id !== '') { // 修改
        dictionaryusernameApi.findById(id).then(response => {
          if (response.flag) {
    ...
    handleDelete(id) {
      this.$confirm('此操作将永久删除已选记录, 是否继续?', '警告', {
        ...
        dictionaryusernameApi.deleteById(id).then(response => {
          this.$message({
            message: response.message,
            type: (response.flag ? 'success' : 'error')
          })
