<template>
  <div>
    ...
    <el-form ref="searchform" inline size="small" :model="searchMap">
      <!-- <el-form-item label="项目组编号">
        <el-input v-model="searchMap.projectinfoid" prop="projectinfoid" clearable placeholder="项目组编号" /></el-form-item>
      ...
      <el-form-item prop="projectinfoid" label="项目组">
        <el-select v-model="searchMap.projectinfoid" style="width:150px;" filterable remote clearable placeholder="请输入关键词搜索并手动选择" :remote-method="getProjectNameList" :loading="searchLoading">
    ...
    >
      ...
      <!-- <el-table-column sortable prop="projectinfoid" label="项目组编号" /> -->
      ...
      <el-table-column sortable align="center" label="检测白名单" width="150">
        <!-- <template slot="header">
          ...
          <el-tooltip placement="top">
            <div slot="content">如果端口在白名单<br>则该ip的检测白名单会置为false</div>
      ...
      <el-table-column sortable align="center" label="提醒白名单" width="150">
        <!-- <template slot="header">
          <span>提醒白名单</span>
          <el-tooltip placement="top">
            <div slot="content">如果端口在白名单<br>则该ip的提醒白名单会置为false</div>
            <i class="el-icon-info" />
          </el-tooltip>
        </template> -->
        <template slot-scope="scope">
    ...
    <el-dialog title="编辑" :visible.sync="dialogFormVisible" width="50%" center :before-close="cleanCache">
      <el-form label-width="110px">
        ...
        <el-form-item prop="projectinfoid" label="项目组">
          {{ projectname }}
          <el-select v-model="pojo.projectinfoid" style="width:400px;" filterable remote clearable placeholder="请输入关键词搜索并手动选择" :remote-method="getProjectNameList" :loading="searchLoading">
            <el-option v-for="item in projectnameList" :key="item.id" :label="item.projectname" :value="item.id" />
          </el-select>
        </el-form-item>
...
<script>
import projectportwhitelistApi from '@/api/projectportwhitelist'
  ...
  methods: {
    ...
    getPortList(query) {
      if (query !== '' && query) {
        ...
        setTimeout(() => {
          this.searchLoading = false
          projectportwhitelistApi.search(1, 10, { 'port': query }).then(response => {
    ...
    handleDeleteAll() {
      if (this.multipleSelection && this.multipleSelection.length) {
        this.$confirm('此操作将永久删除已选记录, 是否继续?', '警告', {
          ...
          projectportwhitelistApi.deleteAllByIds(ids).then(response => {
            this.$message({
    ...
    fetchData() {
      ...
      projectportwhitelistApi.search(this.currentPage, this.pageSize, this.searchMap).then(response => {
    ...
    handleSave() {
      projectportwhitelistApi.update(this.id, this.pojo).then(response => {
        this.$message({
          message: response.message,
          type: (response.flag ? 'success' : 'error')
        })
    ...
    handleDelete(id) {
      this.$confirm('此操作将永久删除已选记录, 是否继续?', '警告', {
        ...
        projectportwhitelistApi.deleteById(id).then(response => {
          this.$message({
            message: response.message,
            type: (response.flag ? 'success' : 'error')
          })
