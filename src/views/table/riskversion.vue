  ...
  <div>
    ...
    <el-form ref="searchform" inline size="small" :model="searchMap">
      ...
      <el-form-item prop="version" label="高危版本">
        <el-select v-model="searchMap.version" style="width:400px;" filterable remote allow-create default-first-option clearable placeholder="请输入关键词搜索并手动选择" :remote-method="getVersionList" :loading="searchLoading">
    ...
    >
      ...
      <el-table-column sortable prop="version" label="高危版本" />
    ...
    <el-dialog title="编辑" :visible.sync="dialogFormVisible" width="50%" center :before-close="cleanCache">
      <el-form label-width="110px">
        ...
        <el-tooltip placement="top">
          ...
          <el-form-item prop="version" label="高危版本">
            <el-select v-model="pojo.version" style="width:400px;" filterable remote allow-create default-first-option clearable placeholder="请输入关键词搜索并手动选择" :remote-method="getVersionList" :loading="searchLoading">
              <el-option v-for="item in versionList" :key="item.id" :label="item.version" :value="item.version" /></el-select>
          </el-form-item>
        </el-tooltip>
        ...
        <!-- <el-tooltip placement="top">
          <div slot="content">具体的版本<br></div>
          <el-form-item label="高危版本"><el-input v-model="pojo.version" style="width:400px;" /></el-form-item>
        </el-tooltip> -->
...
<script>
import riskversionApi from '@/api/riskversion'
...
export default {
  ...
  methods: {
    ...
    getVersionList(query) {
      if (query !== '' && query) {
        ...
        setTimeout(() => {
          ...
          riskversionApi.search(1, 10, { 'version': query }).then(response => {
    ...
    handleDeleteAll() {
      if (this.multipleSelection && this.multipleSelection.length) {
        this.$confirm('此操作将永久删除已选记录, 是否继续?', '警告', {
          ...
          riskversionApi.deleteAllByIds(ids).then(response => {
            this.$message({
    ...
    fetchData() {
      ...
      riskversionApi.search(this.currentPage, this.pageSize, this.searchMap).then(response => {
    ...
    handleSave() {
      riskversionApi.update(this.id, this.pojo).then(response => {
        this.$message({
          message: response.message,
          type: (response.flag ? 'success' : 'error')
        })
    ...
    handleEdit(id) {
      ...
      if (id !== '') { // 修改
        riskversionApi.findById(id).then(response => {
          if (response.flag) {
    ...
    handleDelete(id) {
      this.$confirm('此操作将永久删除已选记录, 是否继续?', '警告', {
        ...
        riskversionApi.deleteById(id).then(response => {
          this.$message({
            message: response.message,
            type: (response.flag ? 'success' : 'error')
          })
