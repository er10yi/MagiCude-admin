...
export default {
  ...
  methods: {
      ...
      if (query !== '' && query) {
        ...
        setTimeout(() => {
          ...
          pluginconfigApi.search(1, 10, { 'name': query }).then(response => {
            this.pluginConfigNameList = response.data.rows.filter(item => {
              return item.name.toLowerCase().indexOf(query.toLowerCase()) > -1
            })
          })
        }, 200)
    ...
    handleDeleteAll() {
      if (this.multipleSelection && this.multipleSelection.length) {
        this.$confirm('此操作将永久删除已选记录, 是否继续?', '警告', {
          ...
          vulnpluginconfigApi.deleteAllByIds(ids).then(response => {
            this.$message({
    ...
    fetchData() {
      ...
      vulnpluginconfigApi.search(this.currentPage, this.pageSize, this.searchMap).then(response => {
    ...
    handleSave() {
      vulnpluginconfigApi.update(this.id, this.pojo).then(response => {
        this.$message({
          message: response.message,
          type: (response.flag ? 'success' : 'error')
        })
    ...
    handleEdit(id) {
      ...
      if (id !== '') { // 修改
        vulnpluginconfigApi.findById(id).then(response => {
          if (response.flag) {
