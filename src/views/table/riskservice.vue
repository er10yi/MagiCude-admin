<template>
  <div>
    ...
    >
      ...
      <el-table-column sortable prop="service" label="高危服务" />
...
<script>
import riskserviceApi from '@/api/riskservice'
...
export default {
  ...
  methods: {
    ...
    getServiceList(query) {
      if (query !== '' && query) {
        ...
        setTimeout(() => {
          ...
          riskserviceApi.search(1, 10, { 'service': query }).then(response => {
    ...
    handleDeleteAll() {
      if (this.multipleSelection && this.multipleSelection.length) {
        this.$confirm('此操作将永久删除已选记录, 是否继续?', '警告', {
          ...
          riskserviceApi.deleteAllByIds(ids).then(response => {
            this.$message({
    ...
    fetchData() {
      ...
      riskserviceApi.search(this.currentPage, this.pageSize, this.searchMap).then(response => {
    ...
    handleSave() {
      riskserviceApi.update(this.id, this.pojo).then(response => {
        this.$message({
          message: response.message,
          type: (response.flag ? 'success' : 'error')
        })
    ...
    handleEdit(id) {
      ...
      if (id !== '') { // 修改
        riskserviceApi.findById(id).then(response => {
          if (response.flag) {
    ...
    handleDelete(id) {
      this.$confirm('此操作将永久删除已选记录, 是否继续?', '警告', {
        ...
        riskserviceApi.deleteById(id).then(response => {
          this.$message({
            message: response.message,
            type: (response.flag ? 'success' : 'error')
          })
