<template>
  <div>
    ...
    <el-form ref="searchform" inline size="small" :model="searchMap">
      <!-- <el-form-item label="检测结果编号">
        <el-input v-model="searchMap.checkresultid" prop="checkresultid" clearable placeholder="检测结果编号" /></el-form-item>
      ...
      <el-form-item prop="checkresultid" label="检测结果">
        <el-select v-model="searchMap.checkresultid" style="width:150px;" filterable remote clearable placeholder="请输入关键词" :remote-method="getCheckResultNameList" :loading="searchLoading">
...
<script>
import checkresultVulnApi from '@/api/checkresultVuln'
...
export default {
  ...
  methods: {
      ...
      if (query !== '' && query) {
        ...
        setTimeout(() => {
          ...
          vulnApi.search(1, 10, { 'name': query }).then(response => {
            this.vulnNameList = response.data.rows.filter(item => {
    ...
    getCheckResultNameList(query) {
      if (query !== '' && query) {
        this.searchLoading = true
        setTimeout(() => {
          this.searchLoading = false
          checkresultApi.search(1, 10, { 'name': query }).then(response => {
            this.checkResultNameList = response.data.rows.filter(item => {
              return item.name.toLowerCase().indexOf(query.toLowerCase()) > -1
            })
          })
        }, 200)
      } else {
        this.checkResultNameList = []
      }
    },
    ...
    handleDeleteAll() {
      if (this.multipleSelection && this.multipleSelection.length) {
        this.$confirm('此操作将永久删除已选记录, 是否继续?', '警告', {
          ...
          checkresultVulnApi.deleteAllByIds(ids).then(response => {
            this.$message({
    ...
    fetchData() {
      ...
      checkresultVulnApi.search(this.currentPage, this.pageSize, this.searchMap).then(response => {
    ...
    handleSave() {
      checkresultVulnApi.update(this.id, this.pojo).then(response => {
        this.$message({
          message: response.message,
          type: (response.flag ? 'success' : 'error')
        })
    ...
    handleEdit(id) {
      ...
      if (id !== '') { // 修改
        checkresultVulnApi.findById(id).then(response => {
          if (response.flag) {
    ...
    handleDelete(id) {
      this.$confirm('此操作将永久删除已选记录, 是否继续?', '警告', {
        ...
        checkresultVulnApi.deleteById(id).then(response => {
          this.$message({
            message: response.message,
            type: (response.flag ? 'success' : 'error')
          })
