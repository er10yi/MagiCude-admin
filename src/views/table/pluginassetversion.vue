<template>
  <div>
    ...
    <el-form ref="searchform" inline size="small" :model="searchMap">
      ...
      <el-form-item prop="assetservice" label="资产版本">
        <el-select v-model="searchMap.assetversion" filterable remote allow-create default-first-option clearable placeholder="请输入关键词搜索并手动选择" :remote-method="getVersion" :loading="searchLoading">
          <el-option v-for="item in versionList" :key="item.id" :label="item.assetversion" :value="item.assetversion" />
        </el-select>
      </el-form-item>
    ...
    <el-dialog title="编辑" :visible.sync="dialogFormVisible" width="50%" center :before-close="cleanCache">
      <el-form label-width="110px">
        ...
        <!-- <el-form-item label="资产版本"><el-input v-model="pojo.assetversion" style="width:400px;" /></el-form-item> -->
        <el-form-item prop="assetversion" label="资产版本">
          <el-select v-model="pojo.assetversion" style="width:400px;" filterable remote allow-create default-first-option clearable placeholder="请输入关键词搜索并手动选择" :remote-method="getService" :loading="searchLoading">
            <el-option v-for="item in serviceList" :key="item.id" :label="item.assetversion" :value="item.assetversion" />
          </el-select>
        </el-form-item>
  ...
  methods: {
    ...
    getVersion(query) {
      if (query !== '' && query) {
        ...
        setTimeout(() => {
          this.searchLoading = false
          pluginassetversionApi.search(1, 10, { 'assetversion': query }).then(response => {
            this.versionList = response.data.rows.filter(item => {
              return item.assetversion.toLowerCase().indexOf(query.toLowerCase()) > -1
            })
          })
        }, 200)
      } else {
    ...
    handleDeleteAll() {
      if (this.multipleSelection && this.multipleSelection.length) {
        this.$confirm('此操作将永久删除已选记录, 是否继续?', '警告', {
          ...
          pluginassetversionApi.deleteAllByIds(ids).then(response => {
            this.$message({
    ...
    fetchData() {
      ...
      pluginassetversionApi.search(this.currentPage, this.pageSize, this.searchMap).then(response => {
    ...
    handleSave() {
      pluginassetversionApi.update(this.id, this.pojo).then(response => {
        this.$message({
          message: response.message,
          type: (response.flag ? 'success' : 'error')
        })
    ...
    handleEdit(id) {
      ...
      if (id !== '') { // 修改
        pluginassetversionApi.findById(id).then(response => {
          if (response.flag) {
