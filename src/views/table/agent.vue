  ...
  <div>
    ...
    <el-form ref="searchform" inline size="small" :model="searchMap">
      <el-form-item label="agent名称">
        <el-input v-model="searchMap.name" prop="name" clearable placeholder="agent名称" style="width:140px;" /></el-form-item>
      <el-form-item label="nmap路径">
        <el-input v-model="searchMap.nmappath" prop="nmappath" clearable placeholder="nmap路径" /></el-form-item>
      <el-form-item label="mass路径">
        <el-input v-model="searchMap.masspath" prop="masspath" clearable placeholder="mass路径" /></el-form-item>
      <el-form-item label="ip地址">
        <el-input v-model="searchMap.ipaddress" prop="ipaddress" clearable placeholder="ip地址" style="width:140px;" /></el-form-item>

      <el-form-item prop="online" label="在线">
        <el-switch v-model="searchMap.online" />
      </el-form-item>

      <el-form-item label="超时次数">
        <el-input v-model="searchMap.timeouts" prop="timeouts" clearable placeholder="超时次数" style="width:100px;" /></el-form-item>
    ...
    >
      ...
      <!-- <el-table-column sortable prop="id" label="agent编号" /> -->
      <el-table-column sortable prop="name" label="agent名称" />
      <el-table-column sortable prop="nmappath" label="nmap路径" />
      <el-table-column sortable prop="masspath" label="mass路径" />
      <el-table-column sortable prop="ipaddress" label="ip地址" />

      <el-table-column sortable prop="online" label="在线">
        <template slot-scope="scope">
          <span>{{ formatBoolean(scope.row.online) }}</span>
        </template>
      </el-table-column>

      <el-table-column sortable prop="timeouts" label="超时次数" />
...
export default {
  ...
  methods: {
    ...
    handleDeleteAll() {
      if (this.multipleSelection && this.multipleSelection.length) {
        this.$confirm('此操作将永久删除已选记录, 是否继续?', '警告', {
          ...
          agentApi.deleteAllByIds(ids).then(response => {
            this.$message({
    ...
    fetchData() {
      ...
      agentApi.search(this.currentPage, this.pageSize, this.searchMap).then(response => {
    ...
    handleSave() {
      agentApi.update(this.id, this.pojo).then(response => {
        this.$message({
          message: response.message,
          type: (response.flag ? 'success' : 'error')
        })
    ...
    handleEdit(id) {
      ...
      if (id !== '') { // 修改
        agentApi.findById(id).then(response => {
          if (response.flag) {
    ...
    handleDelete(id) {
      this.$confirm('此操作将永久删除已选记录, 是否继续?', '警告', {
        ...
        agentApi.deleteById(id).then(response => {
          this.$message({
            message: response.message,
            type: (response.flag ? 'success' : 'error')
          })
